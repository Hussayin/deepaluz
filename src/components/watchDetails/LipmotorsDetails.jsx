  ...
  useEffect(() => {
    if (products?.length) {
      const foundProduct = products.find(
        (item) => item.brend === "LIPMOTORS" && String(item.id) === String(id)
      );
        ...
        <div className=" flex flex-col gap-[20px]">
          ...
          <div className="flex gap-[5px] justify-between items-center">
            <motion.img
              ...
              src={product.logo}
              alt="logo-brend"
              className="rounded-[10px] bg-white w-[90px] p-[5px] h-[50px] text-center object-contain"
            />
            <motion.div
              initial={{ opacity: 0 }}
              whileInView={{ opacity: 1 }}
...
export default LipmotorsDetails;
